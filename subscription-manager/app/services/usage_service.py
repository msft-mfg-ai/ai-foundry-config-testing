...
"""
    ...
    async def _execute_query(self, query: str, timespan: timedelta | None = None) -> list[dict[str, Any]]:
...
"""
            ...
            active_subscriptions = int(sub_results[0].get("Count", 0)) if sub_results else 0
            ...
            return UsageStats(
                ...
                avg_tokens_per_request=month_tokens // month_requests if month_requests > 0 else 0,
                top_consumers=[
                    {"name": c.get("subscription_id", "Unknown"), "tokens": c.get("total_tokens", 0)}
    ...
    async def get_usage_over_time(
        ...
        try:
            ...
            time_filter = f"| where TimeGenerated >= datetime({start_date.isoformat()}) and TimeGenerated < datetime({(end_date + timedelta(days=1)).isoformat()})"
            ...
            for row in results:
                ...
                if isinstance(time_generated, datetime):
                    ...
                    usage_date = datetime.fromisoformat(str(time_generated).replace("Z", "+00:00")).date()
                
                usage.append(
                    DailyUsageSummary(
                        usage_date=usage_date,
                        subscription_id=row.get("SubscriptionId", subscription_id or "all"),
                        total_requests=int(row.get("RequestCount", 0) or 0),
                        total_prompt_tokens=int(row.get("SumPromptTokens", 0) or 0),
                        total_completion_tokens=int(row.get("SumCompletionTokens", 0) or 0),
                        total_tokens=int(row.get("SumTotalTokens", 0) or 0),
                        avg_tokens_per_request=int(row.get("SumTotalTokens", 0) or 0) // max(int(row.get("RequestCount", 1) or 1), 1),
                        models_used=[],
                    )
                )
    ...
    async def get_chart_data(self, days: int = 30) -> dict:
        ...
        daily_usage = await self.get_usage_over_time(start_date=start_date, end_date=end_date)
    ...
    async def get_subscription_chart_data(self, subscription_id: str, days: int = 30) -> dict:
...
"""
    ...
    def _get_mock_daily_usage(
        ...
        while current_date <= end_date:
            ...
            base_tokens = random.randint(10000, 50000) if is_weekend else random.randint(50000, 200000)
            ...
            requests = random.randint(100, 500) if is_weekend else random.randint(300, 1500)
    ...
    def _get_mock_top_consumers(self, limit: int) -> list[dict]:
        ...
        consumers = [
            {"subscription_id": "sub-001", "name": "Production API Access", "total_tokens": 1250000},
            {"subscription_id": "sub-004", "name": "Partner Integration", "total_tokens": 890000},
            {"subscription_id": "sub-002", "name": "Development Team", "total_tokens": 450000},
            {"subscription_id": "sub-005", "name": "Internal Tools", "total_tokens": 320000},
            {"subscription_id": "sub-003", "name": "Testing Environment", "total_tokens": 85000},
        ]

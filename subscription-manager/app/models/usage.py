...
class TokenUsageRecord(BaseModel):
    ...
    subscription_id: str = Field(..., description="Subscription that generated the usage")
...
class DailyUsageSummary(BaseModel):
    ...
    total_tokens: int = Field(default=0, description="Total tokens used")
    avg_tokens_per_request: float = Field(default=0, description="Average tokens per request")
...
class UsageOverTime(BaseModel):
    ...
    daily_usage: list[DailyUsageSummary] = Field(default_factory=list, description="Daily usage data")
...
class UsageStats(BaseModel):
    ...
    total_subscriptions: int = Field(default=0, description="Total number of subscriptions")
    ...
    total_tokens_this_month: int = Field(default=0, description="Tokens used this month")
    top_consumers: list[dict] = Field(default_factory=list, description="Top token consumers")


class UsageQueryParams(BaseModel):
    """Parameters for querying usage data."""

    subscription_id: str | None = Field(default=None, description="Filter by subscription")
